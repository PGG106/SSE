...
void ClearTT() {
    for (uint64_t i = 0; i < TT.paddedSize / sizeof(struct TTEntry); ++i) {
        TTEntry_init(&TT.pTable[i]);
    }
    TT.age = 1;
}
...
void InitTT(uint64_t MB) {
    ...
    TT.numEntries = (hashSize / sizeof(struct TTEntry));
    ...
    TT.paddedSize = (TT.numEntries * sizeof(struct TTEntry) + alignment - 1) / alignment * alignment;
    TT.pTable = (struct TTEntry*)(AlignedMalloc(TT.paddedSize, alignment));
...
void StoreTTEntry(const ZobristKey key, const PackedMove move, int score, int eval, const int bound, const int depth, const bool pv, const bool wasPV) {
    ...
    struct TTEntry* tte = &TT.pTable[index];
...
uint64_t Index(const ZobristKey posKey) {
    ...
    return (uint64_t)((((__uint128_t)(posKey) * (__uint128_t)(TT.numEntries)) >> 64));
#else
    ...
    uint64_t nlo = (uint32_t)TT.numEntries;
    uint64_t nhi = TT.numEntries >> 32;
...
void TTPrefetch(const ZobristKey posKey) {
    prefetch(&TT.pTable[Index(posKey)]);
}
